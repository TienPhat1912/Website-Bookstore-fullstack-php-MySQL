...
// Lịch sử nhập từng lô, kèm giá BQ sau mỗi lô
$lo_nhap = $pdo->prepare("
    ...
    ORDER BY CASE WHEN pn.ngay_nhap = '0000-00-00' OR pn.ngay_nhap IS NULL THEN pn.ngay_tao ELSE pn.ngay_nhap END ASC, pn.ngay_tao ASC
");
$lo_nhap->execute([$id]);
$lo_nhap = $lo_nhap->fetchAll();
...
<div class="page-header">
  <h5>
    ...
    Lịch sử nhập — <span style="color:#f4a261;"><?= htmlspecialchars($sach['ma_sach']) ?></span>
  </h5>
...
<div class="row g-4">
  ...
  <div class="col-lg-7">
    <div class="admin-card">
      <div class="card-title">Lịch sử nhập theo lô (<?= count($lo_nhap) ?> lô)</div>
      <?php if (empty($lo_nhap)): ?>
        <p class="text-muted text-center py-3">Chưa có lô nhập nào.</p>
      <?php else: ?>
          ...
          <table class="table admin-table">
            ...
            <tfoot>
              <tr style="background:#f8f9fa;font-weight:700;">
                <td colspan="3">Tổng nhập</td>
                <td class="text-end"><?= number_format($tong_nhap) ?></td>
                <td colspan="2" class="text-end" style="font-size:.85rem;">
                  Giá vốn BQ hiện tại: <span style="color:#f4a261;"><?= $sach['gia_nhap'] > 0 ? number_format($sach['gia_nhap'],0,',','.').'₫' : '—' ?></span>
                </td>
              </tr>
            </tfoot>
          </table>
  ...
  <div class="col-lg-5">
    <div class="admin-card">
      <div class="card-title">20 đơn xuất gần nhất</div>
