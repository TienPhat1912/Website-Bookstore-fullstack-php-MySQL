...
if ($action === 'complete' && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $phieu = $pdo->prepare("SELECT * FROM phieu_nhap WHERE id = ? AND trang_thai = 'draft'");
    $phieu->execute([$id]);
    $phieu = $phieu->fetch();
    ...
    if ($phieu) {
        ...
        try {
            ...
            $pdo->prepare("
                UPDATE phieu_nhap SET trang_thai = 'done' WHERE id = ?
            ")->execute([$id]);
...
if ($action === 'delete' && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $check = $pdo->prepare("SELECT id FROM phieu_nhap WHERE id = ? AND trang_thai = 'draft'");
    $check->execute([$id]);
    if ($check->fetch()) {
...
if ($action === 'add_item' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $phieu_check = $pdo->prepare("SELECT id FROM phieu_nhap WHERE id = ? AND trang_thai = 'draft'");
...
<div class="page-header">
  <h5>
    ...
    <?php if ($edit_phieu['trang_thai'] === 'done'): ?>
...
<div class="row g-4">
  ...
  <!-- THÊM SÁCH VÀO PHIẾU -->
  <?php if ($edit_phieu['trang_thai'] === 'draft'): ?>
  <div class="col-lg-4">
  ...
  <div class="col-lg-<?= $edit_phieu['trang_thai'] === 'draft' ? '8' : '12' ?>">
    <div class="admin-card">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div style="font-size:.85rem; color:#555;">
          ...
          <?php if ($edit_phieu['ngay_tao']): ?>
            &nbsp;|&nbsp; Hoàn thành: <?= date('d/m/Y H:i', strtotime($edit_phieu['ngay_tao'])) ?>
          <?php endif; ?>
        </div>
      </div>
      ...
      <?php else: ?>
        <div class="table-responsive">
          <table class="table admin-table table-hover">
            ...
            <tfoot>
              <tr>
                <td colspan="<?= $edit_phieu['trang_thai'] === 'draft' ? '5' : '5' ?>"
                ...
                <?php if ($edit_phieu['trang_thai'] === 'draft'): ?><td></td><?php endif; ?>
              </tr>
            </tfoot>
          </table>
        </div>
          ...
          <tr>
            ...
            <td>
              <?php if ($p['trang_thai'] === 'done'): ?>
                ...
                <?php if (null): ?>
                  <br><small class="text-muted"><?= date('d/m/Y', strtotime(null)) ?></small>
                <?php endif; ?>
              <?php else: ?>
